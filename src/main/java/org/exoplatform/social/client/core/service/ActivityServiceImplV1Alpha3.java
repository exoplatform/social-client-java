 ...
 * @since Jun 28, 2011
 */
public class ActivityServiceImplV1Alpha3 extends
                                        ServiceBase<RestActivity, ActivityService<RestActivity>>
    ...
    public RestActivity create(RestActivity newInstance) throws SocialClientLibException {
        ...
        try {
            HttpResponse response = executePost(POST_ACTIVITY_REQUEST_URL,
                                                POLICY.BASIC_AUTH,
                                                newInstance);
            String responseContent = getContent(response);
            handleError(response);
            RestActivity restActivity = SocialJSONDecodingSupport.parser(RestActivity.class,
                                                                         responseContent);
            return restActivity;
    ...
    public RestActivity delete(RestActivity existingInstance) throws SocialClientLibException {
        final String DELETE_ACTIVITY_REQUEST_URL = BASE_URL + "activity/destroy/"
                + existingInstance.getId() + ".json";
        try {
    ...
    public RealtimeListAccess<RestActivity> getActivityStream(RestIdentity restIdentity,
                                                              QueryParams queryParams) throws SocialClientLibException {
        return new ActivitiesRealtimeListAccessV1Alpha3(restIdentity,
                                                        ActivityType.ACTIVITY_STREAM,
                                                        queryParams);
    }
    ...
    public RealtimeListAccess<RestActivity> getSpacesActivityStream(RestIdentity userRestIdentity,
                                                                    QueryParams queryParams) throws SocialClientLibException {
        return new ActivitiesRealtimeListAccessV1Alpha3(userRestIdentity,
                                                        ActivityType.USER_SPACE_ACTIVITIES,
                                                        queryParams);
    }
    ...
    public RealtimeListAccess<RestActivity> getConnectionsActivityStream(RestIdentity restIdentity,
                                                                         QueryParams queryParams) throws SocialClientLibException {
        return new ActivitiesRealtimeListAccessV1Alpha3(restIdentity,
                                                        ActivityType.CONNECTIONS_ACTIVITIES,
                                                        queryParams);
    }
    ...
    public RestComment createComment(RestActivity existingRestActivity, RestComment newRestComment) throws SocialClientLibException {
        final String CREATE_COMMENT_REQUEST_URL = BASE_URL + "activity/"
                + existingRestActivity.getId() + "/comment.json";
        try {
            HttpResponse response = executePost(CREATE_COMMENT_REQUEST_URL,
                                                POLICY.BASIC_AUTH,
                                                newRestComment);
            String responseContent = getContent(response);
            handleError(response);
            RestComment restComment = SocialJSONDecodingSupport.parser(RestComment.class,
                                                                       responseContent);
            return restComment;
        } catch (SocialHttpClientException e) {
            throw new ServiceException(ActivityServiceImplV1Alpha3.class, e.getMessage(), e);
    ...
    public RestComment getComment(String commentId) throws SocialClientLibException {
        ...
        // SocialHttpClientSupport.executeGet(GET_ACTIVITY_REQUEST_URL,POLICY.BASIC_AUTH);
        ...
        // } catch (Exception e) {
        // throw new
        // ServiceException(ActivityServiceImplV1Alpha1.class,"invalid response",null);
        // }
        // }
        // } catch (Exception e) {
        // throw new ServiceException(ActivityServiceImplV1Alpha1.class,
        // "There's error when execute request",null);
        // }
    }
    ...
    public RestComment deleteComment(RestComment existingRestComment) throws SocialClientLibException {
        final String DELETE_COMMENT_REQUEST_URL = BASE_URL + "activity/"
                + existingRestComment.getActivityId() + "/comment/destroy/"
    ...
    public RestLike like(RestActivity existingRestActivity) throws SocialClientLibException {
        final String LIKE_ACTIVITY_REQUEST_URL = BASE_URL + "activity/"
                + existingRestActivity.getId() + "/like.json";
        try {
            ...
            if ((Boolean) responseJson.get("liked")) {
                ...
                throw new ServiceException(ActivityServiceImplV1Alpha3.class,
                                           "invalid response",
                                           null);
            }
        } catch (SocialHttpClientException e) {
    ...
    public RestLike unlike(RestActivity existingRestActivity) throws SocialClientLibException {
        final String LIKE_ACTIVITY_REQUEST_URL = BASE_URL + "activity/"
                + existingRestActivity.getId() + "/like/destroy.json";
        try {
            HttpResponse response = executePost(LIKE_ACTIVITY_REQUEST_URL, POLICY.BASIC_AUTH);
            handleError(response);
            String responseContent = getContent(response);
    ...
    public RestActivity get(String activityId, QueryParams queryParams) throws SocialClientLibException {
        ...
        final String GET_ACTIVITY_REQUEST_URL = BASE_URL + "activity/" + activityId + ".json?"
                + queryParams.buildQuery();
        try {
            HttpResponse response = executeGet(GET_ACTIVITY_REQUEST_URL, POLICY.BASIC_AUTH);
            handleError(response);
